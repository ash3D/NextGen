...
const vec4t operator*(const vec4t& v, const mat4& M)
{
	vec4t u;
...
{
    nv_scalar x2 = q.x * q.x * nv_two;
    nv_scalar y2 = q.y * q.y * nv_two;
    nv_scalar z2 = q.z * q.z * nv_two;

    M.a00 = nv_one - y2 - z2;
    M.a11 = nv_one - x2 - z2;
    M.a22 = nv_one - x2 - y2;

    nv_scalar wz = q.w*q.z, xy = q.x*q.y;        
    M.a01 = nv_two * (xy - wz);
    M.a10 = nv_two * (xy + wz);

    nv_scalar wx = q.w*q.x, yz = q.y*q.z;
    M.a12 = nv_two * (yz - wx);
    M.a21 = nv_two * (yz + wx);

    nv_scalar wy = q.w*q.y, xz = q.x*q.z;
    M.a02 = nv_two * (xz + wy);
    M.a20 = nv_two * (xz - wy);
    return M;
}
...
{
    nv_scalar tr = M.a00 + M.a11 + M.a22;
      
    // check the diagonal
    if ( tr > nv_zero) 
    {
        // Diagonal is positive.
        nv_scalar s = sqrtf(tr + nv_one);
        q.w = s * nv_zero_5;
        s = nv_zero_5 / s;
        q.x = (M.a21 - M.a12) * s;
        q.y = (M.a02 - M.a20) * s;
    ...
    {                
        ...
        nv_scalar s = sqrtf( M[i][i] - ( M[j][j] + M[k][k] )) + nv_one;
...
quat & mat4::get_rot(quat& q) const
{
    ...
    {                
        ...
        nv_scalar s = sqrtf(( (*this)[i][i] - ( (*this)[j][j] + (*this)[k][k] )) + nv_one );
