...
float4 main(in float3 viewDir : ViewDir, in float2 uv : UV) : SV_TARGET
{
	...
	const float3 color = Lit(albedo * tex.Sample(TerrainSamplers::albedo, uv), .5f, F0(1.55f), mul(-terrainWorldXform[2], viewXform), normalize(viewDir), sun.dir, sun.irradiance);
