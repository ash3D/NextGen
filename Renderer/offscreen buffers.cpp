...
inline void OffscreenBuffers::MarkupAllocation()
{
	...
	// shaders
	{
		...
		shaders.persistent.HDRCompositeSurface.offset = 0;
		shaders.allocationSize = shaders.persistent.HDRCompositeSurface.size = HDRCompositeSurfaceAllocation.SizeInBytes;
		...
		shaders.overlapped.bokeh.HDRInputSurface.offset = alignedBokehOffset + bokehSuballocation[BokehLayout_HDRInputSurface].Offset;
		shaders.overlapped.bokeh.HDRInputSurface.size = bokehSuballocation[BokehLayout_HDRInputSurface].SizeInBytes;
		shaders.overlapped.bokeh.DOFOpacityBuffer.offset = alignedBokehOffset + bokehSuballocation[BokehLayout_DOFOpacityBuffer].Offset;
		shaders.overlapped.bokeh.DOFOpacityBuffer.size = bokehSuballocation[BokehLayout_DOFOpacityBuffer].SizeInBytes;
		shaders.overlapped.bokeh.COCBuffer.offset = alignedBokehOffset + bokehSuballocation[BokehLayout_COCBuffer].Offset;
		shaders.overlapped.bokeh.COCBuffer.size = bokehSuballocation[BokehLayout_COCBuffer].SizeInBytes;
		shaders.overlapped.bokeh.dilatedCOCBuffer.offset = alignedBokehOffset + bokehSuballocation[BokehLayout_DilatedCOCBuffer].Offset;
		shaders.overlapped.bokeh.dilatedCOCBuffer.size = bokehSuballocation[BokehLayout_DilatedCOCBuffer].SizeInBytes;
		shaders.overlapped.bokeh.halfresDOFSurface.offset = alignedBokehOffset + bokehSuballocation[BokehLayout_HalfresDOFSurface].Offset;
		shaders.overlapped.bokeh.halfresDOFSurface.size = bokehSuballocation[BokehLayout_HalfresDOFSurface].SizeInBytes;
		...
		shaders.overlapped.lum.bloomUpChain.offset = alignedLumOffset + lumSuballocation[LumLayout_BloomUpChain].Offset;
		shaders.overlapped.lum.bloomUpChain.size = lumSuballocation[LumLayout_BloomUpChain].SizeInBytes;
		shaders.overlapped.lum.bloomDownChain.offset = alignedLumOffset + lumSuballocation[LumLayout_BloomDownChain].Offset;
		shaders.overlapped.lum.bloomDownChain.size = lumSuballocation[LumLayout_BloomDownChain].SizeInBytes;
		shaders.overlapped.lum.LDRSurface.offset = alignedLumOffset + lumSuballocation[LumLayout_LDRSurface].Offset;
		shaders.overlapped.lum.LDRSurface.size = lumSuballocation[LumLayout_LDRSurface].SizeInBytes;

		shaders.allocationSize += max(bokehPadding + bokehAllocation.SizeInBytes, lumPadding + lumAllocation.SizeInBytes);
	}
}
...
inline void OffscreenBuffers::DestroyBuffers()
{
	...
	shaders.persistent.HDRCompositeSurface.resource.Reset();
	shaders.overlapped.bokeh.HDRInputSurface.resource.Reset();
	shaders.overlapped.bokeh.DOFOpacityBuffer.resource.Reset();
	shaders.overlapped.bokeh.COCBuffer.resource.Reset();
	shaders.overlapped.bokeh.dilatedCOCBuffer.resource.Reset();
	shaders.overlapped.bokeh.halfresDOFSurface.resource.Reset();
	shaders.overlapped.lum.bloomUpChain.resource.Reset();
	shaders.overlapped.lum.bloomDownChain.resource.Reset();
	shaders.overlapped.lum.LDRSurface.resource.Reset();
}
...
inline void OffscreenBuffers::ConstructBuffers()
{
	// create placed resources into shared heaps
	{
		...
		CheckHR(device->CreatePlacedResource(
			...
			shaders.persistent.HDRCompositeSurface.offset,
			&resourceDesc.HDRCompositeSurface(),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			NULL,
			IID_PPV_ARGS(shaders.persistent.HDRCompositeSurface.resource.GetAddressOf())
		));
		CheckHR(device->CreatePlacedResource(
			shaders.VRAMBackingStore.Get(),
			shaders.overlapped.bokeh.HDRInputSurface.offset,
			&resourceDesc.HDRInputSurface(),
			D3D12_RESOURCE_STATE_RESOLVE_DEST,
			NULL,
			IID_PPV_ARGS(shaders.overlapped.bokeh.HDRInputSurface.resource.GetAddressOf())
		));

		// create DOF opacity buffer
		CheckHR(device->CreatePlacedResource(
			shaders.VRAMBackingStore.Get(),
			shaders.overlapped.bokeh.DOFOpacityBuffer.offset,
			&resourceDesc.DOFOpacityBuffer(),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			NULL,
			IID_PPV_ARGS(shaders.overlapped.bokeh.DOFOpacityBuffer.resource.GetAddressOf())
		));

		// create fullres CoC buffer
		CheckHR(device->CreatePlacedResource(
			shaders.VRAMBackingStore.Get(),
			shaders.overlapped.bokeh.COCBuffer.offset,
			&resourceDesc.COCBuffer(),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			NULL,
			IID_PPV_ARGS(shaders.overlapped.bokeh.COCBuffer.resource.GetAddressOf())
		));

		// create halfres dilated CoC buffer
		CheckHR(device->CreatePlacedResource(
			shaders.VRAMBackingStore.Get(),
			shaders.overlapped.bokeh.dilatedCOCBuffer.offset,
			&resourceDesc.DilatedCOCBuffer(),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			NULL,
			IID_PPV_ARGS(shaders.overlapped.bokeh.dilatedCOCBuffer.resource.GetAddressOf())
		));

		// create halfres DOF color surface
		CheckHR(device->CreatePlacedResource(
			shaders.VRAMBackingStore.Get(),
			shaders.overlapped.bokeh.halfresDOFSurface.offset,
			&resourceDesc.HalfresDOFSurface(),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			NULL,
			IID_PPV_ARGS(shaders.overlapped.bokeh.halfresDOFSurface.resource.GetAddressOf())
		));
		...
		// create bloom chains
		{
			...
			CheckHR(device->CreatePlacedResource(
				shaders.VRAMBackingStore.Get(),
				shaders.overlapped.lum.bloomUpChain.offset,
				&resourceDesc.BloomUpChain(),
				D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
				NULL,
				IID_PPV_ARGS(shaders.overlapped.lum.bloomUpChain.resource.GetAddressOf())
			));

			// down
			CheckHR(device->CreatePlacedResource(
				shaders.VRAMBackingStore.Get(),
				shaders.overlapped.lum.bloomDownChain.offset,
				&resourceDesc.BloomDownChain(),
				D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
				NULL,
				IID_PPV_ARGS(shaders.overlapped.lum.bloomDownChain.resource.GetAddressOf())
			));
		}
		...
		CheckHR(device->CreatePlacedResource(
			shaders.VRAMBackingStore.Get(),
			shaders.overlapped.lum.LDRSurface.offset,
			&resourceDesc.LDRSurface(),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
			NULL,
			IID_PPV_ARGS(shaders.overlapped.lum.LDRSurface.resource.GetAddressOf())
		));
	}
	...
	// fill descriptor heaps
	{
		...
		device->CreateRenderTargetView(ROPs.overlapped.rendertarget.resource.Get(), NULL, CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvStoreBase, SCENE_RTV, rtvSize));

		// fill DSV store
		device->CreateDepthStencilView(ROPs.persistent.ZBuffer.resource.Get(), NULL, dsvStore->GetCPUDescriptorHandleForHeapStart());
		...
		// fill DOF RTV store
		{
			...
			// near layers
			device->CreateRenderTargetView(ROPs.overlapped.postFX.DOFLayers.resource.Get(), &rtvDesc, CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvStoreBase, DOF_NEAR_RTV, rtvSize));

			// far layers
			rtvDesc.Texture2DArray.FirstArraySlice = 2;
			device->CreateRenderTargetView(ROPs.overlapped.postFX.DOFLayers.resource.Get(), &rtvDesc, CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvStoreBase, DOF_FAR_RTV, rtvSize));
		}

		// fill lens flare RTV store
		device->CreateRenderTargetView(ROPs.overlapped.postFX.lensFlareSurface.resource.Get(), NULL, CD3DX12_CPU_DESCRIPTOR_HANDLE(rtvStoreBase, LENS_FLARE_RTV, rtvSize));
